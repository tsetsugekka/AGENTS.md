...
![规则组](https://img.shields.io/badge/AGENTS.md-7%20%E8%A7%84%E5%88%99%E7%BB%84-2563eb)
...
七份文件是主题化规则组，而不是互相排斥的配置。通常可以这样组合：

1. 先加入 `temporary-files-and-project-structure-hygiene`，建立所有任务的文件生命周期底线。
2. 需要并行处理时加入 `multi-agent-delegation-and-model-routing`。
3. 有 GitHub 变更时加入 `github-publish-discipline`。
4. 使用外部 API 或抓取数据时加入 API key 与网络抓取规则。
5. 生成 Office、PDF 或其他带元数据的文件时加入匿名元数据规则。
6. 只有在确实需要长任务提醒时才加入 Telegram 通知规则。
