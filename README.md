...
![规则组](https://img.shields.io/badge/AGENTS.md-9%20%E8%A7%84%E5%88%99%E7%BB%84-2563eb)
...
> 将一份全局工作说明拆成九个可独立复用的 `AGENTS.md` 规则组，覆盖多 Agent 协作、模型路由、临时文件、Caffeine 保持唤醒、Telegram 通知、GitHub 发布、API key、网络抓取和文档隐私元数据。
...
| 文件 | 主要关注点 | 适合单独复用的场景 |
|---|---|---|
| [`final-response-status/AGENTS.md`](final-response-status/AGENTS.md) | 明确最终回答的实际状态，有未完成或阻塞时简要说明 | Codex / Claude Code |
...
九份文件是主题化规则组，而不是互相排斥的配置。通常可以这样组合：
...
- 这些文件来自同一份全局说明的九个一级规则组，文件内容保持对应规则组的完整语义。
