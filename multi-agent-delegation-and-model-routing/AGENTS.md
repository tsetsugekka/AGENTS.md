...
- For difficult but bounded work with manageable context, first evaluate `gpt-6-astra` with `low` or `medium` reasoning effort: use `low` for moderate complexity with controlled risk, and `medium` for longer context, higher reliability, or work that would previously have used `gpt-5.6-sol` with `high` or `xhigh`.
- Keep `gpt-5.6-sol` with `high` or `xhigh` as an alternative route when Sol has better domain fit, Astra is unavailable, cost or latency is preferable, or the subtask is a good delegation target; do not treat it as the default high-tier entry for difficult work.
- For high-importance, ambiguous, long-context, high-risk, or higher-reliability work, use `gpt-6-astra` with `high` or `xhigh`. Use `gpt-5.6-terra` with `high` reasoning effort when implementation reliability or tool execution is the main concern.
...
- Work that remains beyond Astra `low`/`medium`—such as ambiguous requirements, cross-source synthesis, complex UI recovery, high-stakes interpretation, or irreversible operations—should be routed to `gpt-5.6-sol` (`high`/`xhigh`) or `gpt-6-astra` (`high`/`xhigh`) according to task-level judgment, rather than split mechanically by step count.
