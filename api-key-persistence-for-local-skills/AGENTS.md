...
- When adding or rotating a password, API key, token, or other credential, use a local hidden-input prompt or a confirmed interactive terminal so the user enters it directly. If the current tool cannot reliably receive interactive input, provide a user-run hidden-input command or a user-controlled secure edit path; never ask the user to send the credential in chat or type it at an ordinary shell prompt. Keep plaintext out of command arguments, shell history, logs, and tool output; report only save and validation results.
