...
- Do not enable notifications automatically every turn; the existing Stop hook handles sending, consuming the marker after success, and recording a redacted receipt. See [README.md](README.md) for its execution contract. Read credentials only from `TG_BOT_TOKEN` and `TG_CHAT_ID`; never write tokens to files.
