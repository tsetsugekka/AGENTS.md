...
![Rule groups](https://img.shields.io/badge/AGENTS.md-7%20rule%20groups-2563eb)
...
The seven files are thematic rule groups, not mutually exclusive configurations. A practical composition is:

1. Start with `temporary-files-and-project-structure-hygiene` for the baseline file lifecycle.
2. Add `multi-agent-delegation-and-model-routing` when parallel work is useful.
3. Add `github-publish-discipline` whenever the task changes GitHub state.
4. Add the API-key and network-fetching rules when external APIs or data sources are involved.
5. Add the anonymous-metadata rules when creating Office, PDF, or other packaged artifacts.
6. Add Telegram notification rules only when meaningful long-task reminders are actually needed.
